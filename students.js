const studentsScore = [
  ...
  const maxScore = Math.max(...studentsScore.map(student => student.score));
  const topStudents = studentsScore.filter(student => student.score === maxScore);
  
  topStudents.forEach(student => {
    console.log(`Congratulations to ${student.name} for achieving the highest score of ${maxScore}!`);
  });
  module.exports = studentsScore;
